...
function ControlPanel({ handleJoin, globeElement, setError }) {
  return (
      ...
      <button
        onClick={() => {
          if (globeElement) {
            ...
            try {
              globeElement.current.pointOfView(globeElement.currentuserLocation, POV_POSITION_TIME);
            } catch (error) {
              console.error("Error during setting point of view:", error);
              setError("Error", "Error during setting point of view", false);
            }
          }
        }}
      >
